...
import com.regex.FBInitialize;
import com.regex.FireBaseDto;
 ...
 * @author nvhuy9527
 */
public class CoolTextualExecutionListener extends FBInitialize implements ExecutionListener, ExecutionListenerFactory {
	...
	public CoolTextualExecutionListener(String message, Configuration configuration, Results results, String key, FirebaseDatabase defaultDatabase) {
		this.header = ((message != null) ? message.concat("\n") : "") + "Generating: "
				+ configuration.getOutputFolder().getName();
	...
	private synchronized void print() {
		...
		if (isEvaluatorCached) {
			...
			System.out.printf("[%s] %.2f%%  | %d/%d | ETA: %s | CR: %.2f\n", progress(doneAll), percAll, jobDone,
					jobTotal, eta, evaluator.getRatio());
			percent = percAll;
			process = jobDone + "/" + jobTotal;
		} else {
		...
		regex = printRegex(best.getSolution());

		DatabaseReference ref = defaultDatabase.getReference();
		DatabaseReference usersRef = ref.child(key);

		DatabaseReference mDbRef = defaultDatabase.getReference(key + "/regexList");
		mDbRef.addValueEventListener(new ValueEventListener() {
			@Override
			public void onDataChange(DataSnapshot dataSnapshot) {
				regexList = dataSnapshot.getValue(List.class);
			}
		...
		fireBaseDto.setPercent(percent);
		fireBaseDto.setProcess(process);
		fireBaseDto.setRegex(regex);
		if (regexList.indexOf(regex) == -1) {
			regexList.add(regex);
		}
		fireBaseDto.setRegexList(regexList);

		usersRef.setValueAsync(fireBaseDto);
	...
	@Override
	public void logGeneration(RunStrategy strategy, int generation, Node best, double[] fitness,
			List<Ranking> population) {
		int jobId = strategy.getConfiguration().getJobId();
		int done = 20 * generation / strategy.getConfiguration().getEvolutionParameters().getGenerations();
		double perc = Math.round(
				1000 * generation / (double) strategy.getConfiguration().getEvolutionParameters().getGenerations())
				/ 10f;
		...
		Objective learningObjective = PerformancesFactory.buildObjective(Context.EvaluationPhases.LEARNING,
				strategy.getConfiguration());
		double[] learningPerformance = learningObjective.fitness(population.get(0).getTree());
		PerformacesObjective.populatePerformancesMap(learningPerformance,
				generationBestSolution.getLearningPerformances(), isFlagging);
		...
		String print = String.format("[%s] %.2f%% g: %d j: %d f: %s d: %.2f%% ", progress(done), perc, generation,
				jobId, printArray(fitness), Utils.diversity(population));
		synchronized (screen) {
			screen.put(jobId, print);
			print();
		}

		results.addCharachterEvaluated(
				strategy.getContext().getCurrentDataSet().getNumberOfChars() * population.size());
	}
	...
	public void evolutionComplete(RunStrategy strategy, int generation, List<Ranking> population) {
		 ...
		 */
		Objective trainingObjective = PerformancesFactory.buildObjective(Context.EvaluationPhases.TRAINING,
				strategy.getConfiguration());
		Objective validationObjective = PerformancesFactory.buildObjective(Context.EvaluationPhases.VALIDATION,
				strategy.getConfiguration());
		Objective learningObjective = PerformancesFactory.buildObjective(Context.EvaluationPhases.LEARNING,
				strategy.getConfiguration());
		for (int i = 0; i < population.size(); i++) {
			...
			if (i == 0) {
				...
				PerformacesObjective.populatePerformancesMap(trainingPerformace,
						finalSolution.getTrainingPerformances(), isFlagging);
				PerformacesObjective.populatePerformancesMap(validationPerformance,
						finalSolution.getValidationPerformances(), isFlagging);
				PerformacesObjective.populatePerformancesMap(learningPerformance,
						finalSolution.getLearningPerformances(), isFlagging);
			}
